...
//! (`anthropic` | `kimi` | `glm`); individual fields can be overridden with
//! `CWI_AGENT_BASE_URL`, `CWI_AGENT_MODEL`, `CWI_AGENT_API_KEY`, etc.
...
pub struct Provider {
    pub name: String,
    /// Base origin, no trailing slash. `/v1/messages` is appended.
    pub base_url: String,
    pub api_key: String,
    pub model: String,
    pub max_tokens: u32,
    /// Ask for summarized reasoning (`thinking.display = "summarized"`).
    pub thinking: bool,
...
impl Provider {
    ...
    pub fn build(preset: &str, model_override: Option<String>) -> Self {
        // (base_url, model, auth, thinking_default, beta)
        let (base_url, model, auth, thinking, beta) = match preset {
            "kimi" | "moonshot" => (
