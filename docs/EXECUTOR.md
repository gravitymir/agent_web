...
>
> Шаги 1–2 бьют в гостя **без** сессионной куки, поэтому `/api/health` и
> `/api/drain/begin` — public-пути (обходят гейт `CWI_AUTH`, см. `is_public` в
> `src/auth.rs`). Иначе под `CWI_AUTH=1` шаг 1 вернул бы 401 и гость не перестал
> бы принимать новые ходы во время слива.
